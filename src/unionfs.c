...
end_file:
    ...
    if(is_file_type(destpath, TYPE_DIR)){
        ...
        DIR* dirp = getDirents(destpath, &darr, &num);
        int fd = dirfd(dirp);
        log_debug("%s retrieve internal directory fd:%d of %s", function, fd, destpath);
        if(darr_list[fd] != NULL){
            clearItems(&darr_list[fd]);
        }
        clearItems(&darr);
        int dir_num;
        darr = listDir(destpath, &dir_num);
        darr_list[fd] = darr;

        //here fd is generated by dirfd(dirp) which is internally used inside DIR* stream, it is different from fd returned by open. 
        //this function is not process-safe/thread-safe, if multiple processes/threads get access to the same path(folder), the fd here will change and readdir could not get the correct content associated to one specfic path
        INITIAL_SYS(closedir)
        real_closedir(dirp);
    }
